...
export const migratorOptionsValidator = z.object({
  ...
  tableName: z.string().regex(/^[a-zA-Z_][a-zA-Z0-9_]*$/, 'tableName must be a valid SQL identifier').default('db_evolutions'),
...
export interface RollbackSuccessResult { status: "success"; rolledBack: number[] }
export interface RollbackFailureResult { status: "failure"; error: string }
